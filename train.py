...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        "--batch_frequency", type=int, default=50, help="Log images every n batches."
    )
    parser.add_argument(
...
def training_function(config: Dict, args: argparse.Namespace):
    ...
    if config["scheduler"]["scheduler_type"] == "linear-warmup_cosine-decay":
        scheduler_ae = CosineAnnealingLR(optimizer_ae, warmup_steps, min_learning_rate)
        scheduler_disc = CosineAnnealingLR(
            optimizer_disc, warmup_steps, min_learning_rate
        )
    else:
        scheduler_ae = ReduceLROnPlateau(
            optimizer_ae, factor=0.5, patience=3
        )  # Reduce LR on plateau
        scheduler_disc = ReduceLROnPlateau(optimizer_disc, factor=0.5, patience=3)
